...
function handleRecord() {
  if (!recording) {
    ...
    document
      .getElementById("record-pic")
      .setAttribute("src", "/static/img/stop_red.png");
    axios
      .get("/api/userinfo")
      .then(res => {
    ...
    document
      .getElementById("record-pic")
      .setAttribute("src", "/static/img/record.png");
  }
  recording = !recording;
}
...
function appendSpawn(spawnInfo, hue) {
  ...
  if (spawnInfo.type === "meteor") {
    ...
    spawn.setAttribute(
      "src",
      `/static/img/${spawnInfo.type}${spawnInfo.num}.gif?x=${Date.now()}`
    );
  } else {
    spawn.setAttribute(
      "src",
      `/static/img/${spawnInfo.type}${spawnInfo.num}.png`
    );
  }
...
window.onload = () => {
  ...
  document.onmousemove = () => {
    if (!prompt) {
      ...
      timeout = setTimeout(() => {
        ...
        if (!recording) {
          topRight.style.transition =
            "opacity 4s ease, transform 1s ease-in-out";
  ...
  document.onkeypress = e => {
    ...
    const modal = document.getElementById("modal");
    if (
      !(joinInput === document.activeElement) &&
      modal.style.display === "none"
    ) {
  ...
  axios
    ...
    .then(res => {
      ...
      if (userInfo !== "anon") {
        ...
        axios.post("/api/join").catch(error => {
          console.log(error);
        });
      }
    })
...
socket.on("start record", (name, time) => {
  ...
  if (document.getElementById("logout")) {
    ...
    document.getElementById(
      "record-text-nli"
    ).innerText = `${name} is recording`;
    document.getElementById("record-pic-nli").style.width = "18px";
    document
      .getElementById("record-pic-nli")
      .setAttribute("src", "/static/img/record.png");
  }
});
